...
test('Buffer#bind/unbind with index', t => {
  for (const device of getWebGLTestDevices()) {
    ...
    device.gl2.bindBufferBase(buffer.target, 0, buffer.handle);
    t.ok(buffer instanceof Buffer, `${device.info.type} Buffer bind/unbind with index successful`);
    device.gl2.bindBufferBase(buffer.target, 0, null);
...
test('Buffer#construction', t => {
  for (const device of getWebGLTestDevices()) {
    ...
    t.ok(buffer.target === GL.ARRAY_BUFFER, `${device.info.type} Buffer(ARRAY_BUFFER) successful`);
    buffer.destroy();
...
test('Buffer#write', t => {
  ...
  for (const device of getWebGLTestDevices()) {
    ...
    buffer = device.createBuffer({usage: Buffer.VERTEX, byteLength: 12});
    buffer.write(data);
    if (device.isWebGL2) {
